...
## 关联仓库
- **前端代码仓库**：`../linkinfra-web`（remote `ye4293/linkinfra-web`，Next.js 14 + next-auth v5）。这是本项目实际使用的前端代码库，非仓库内置的 `web/default/`。
...
若本地 `output/` 保存了旧 Go 源码草稿，`go vet ./...` 会同时扫描它们。保留这些本地文件，改在只包含当前发布代码的隔离检出目录执行完整检查，不将草稿报错误判为业务代码失败。
