...
func TestMimoLive(t *testing.T) {
	key := os.Getenv("MIMO_TEST_API_KEY")
	if key == "" {
		t.Skip("MIMO_TEST_API_KEY is not set")
	}
	...
	for _, tc := range []struct {
		...
		{"chat", "/v1/chat/completions", `{"model":"mimo-v2.5","max_tokens":128,"thinking":{"type":"disabled"},"messages":[{"role":"user","content":"Reply with only OK."}]}`, "[DONE]"},
		{"responses", "/v1/responses", `{"model":"mimo-v2.5","max_output_tokens":128,"reasoning":{"effort":"none"},"input":"Reply with only OK."}`, "response.completed"},
		{"messages", "/v1/messages", `{"model":"mimo-v2.5","max_tokens":128,"thinking":{"type":"disabled"},"messages":[{"role":"user","content":"Reply with only OK."}]}`, "message_stop"},
	} {
		for _, stream := range []bool{false, true} {
			...
			t.Run(name, func(t *testing.T) {
				...
				meta := &util.RelayMeta{
					ChannelType:    common.ChannelTypeMimo,
